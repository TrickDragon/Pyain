...
class MonsterIndex:
    ...
    def display_main(self, dt):
        ...
        draw_bar(self.display_surface, healthbar_rect, monster.health, monster.get_stat('max_health'), COLORS['red'],
                 COLORS['black'], 2)
        hp_text = self.fonts['regular'].render(f'HP: {int(monster.health)}/{int(monster.get_stat('max_health'))}',
                                               False, COLORS['white'])
        ...
        draw_bar(self.display_surface, energybar_rect, monster.energy, monster.get_stat('max_energy'), COLORS['blue'],
                 COLORS['black'], 2)
        ep_text = self.fonts['regular'].render(f'EP: {int(monster.energy)}/{int(monster.get_stat('max_energy'))}',
                                               False, COLORS['white'])
        ...
        for index, (stat, value) in enumerate(monster_stats.items()):
            ...
            draw_bar(self.display_surface, bar_rect, value, self.max_stats[stat] * monster.level, COLORS['white'],
                     COLORS['black'])
